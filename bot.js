...
bot.on("messageCreate", async msg => {
  ...
  */
  ...
  async function linkIgnToDiscordId(rebuild = false) {
    ...
    if (!rebuild && pilotIdList.indexOf(msg.author.id) != -1) {
      alreadyAdded = true
      msg.channel.createMessage({
        embed: {
          color: 0x7188d9,
    ...
    if (!alreadyAdded && !rebuild && msg.content.length == 6) {
      const args = msg.content.trim().split(/ +/g);
      var clanName = "";
      var memberobj = guildMembers[msg.author.id]
      ...
      messagesPromise.then(function (MessagesList) {
        ...
        for (var index = 0; index < MessagesList.length; index++) {
          ...
          if (args[0].length == 6) {
            ...
            pilotIdList.push(msg.author.id);
            linkers.push({
              username: MessagesList[index].author.username,
        ...
        bot.createMessage("463134158403796992",
          {
            embed: {
              ...
              fields: [
                {
                  name: "Rebuild linkage status",
                  value: "Linkage rebuid successful. Initiated by - " + msg.author.username
                }
              ]
            }
          })
